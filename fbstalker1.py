...
def parseTimeline(html, username):
    ...
    if (len(photosCommentedCount) > 1):
        ...
        photosCommentedCount, photosCommentedUser = zip(
            *sorted(zip(photosCommentedCount, photosCommentedUser), reverse=True))
        count = 0
...
def downloadTimeline(username):
    ...
    while (match == False):
        ...
        if lastCount == lenOfPage:
            ...
            try:
                clickLink = WebDriverWait(driver, 1).until(
                    lambda driver: driver.find_element_by_link_text('Show Older Stories'))
                if clickLink:
            ...
            if z['src'].endswith('.jpg'):
                ...
                if m:
                    ...
                    if m:
                        ...
                        print "[*] Extracting Data from Photo Page: " + username3
                        tmpStr = []
                        tmpStr.append([str(uid), repr(zlib.compress(normalize(z['alt']))), normalize(z['src']),
                                       normalize(i['href']), normalize(username3)])
            ...
            if y['src'].endswith('.jpg'):
                ...
                if m:
                    ...
                    if m:
                        ...
                        print "[*] Extracting Data from Photo Page: " + username3
                        tmpStr = []
                        tmpStr.append([str(uid), repr(zlib.compress(normalize(y['alt']))), normalize(y['src']),
                                       normalize(i['href']), normalize(username3)])
                        write2Database('photosLiked', tmpStr)
            ...
            if x['src'].endswith('.jpg'):
                ...
                if m:
                    ...
                    if m:
                        ...
                        print "[*] Extracting Data from Photo Page: " + username3
                        tmpStr = []
                        tmpStr.append([str(uid), repr(zlib.compress(normalize(x['alt']))), normalize(x['src']),
                                       normalize(i['href']), normalize(username3)])
                        write2Database('photosLiked', tmpStr)
...
def parseFriends(html):
    mthList = ['january', 'february', 'march', 'april', 'may', 'june', 'july', 'august', 'september', 'october',
               'november', 'december']
    if len(html) > 0:
        soup = BeautifulSoup(html)
...
def mainProcess(username):
    ...
    for x in userID:
        ...
        if "/" not in filename:
            ...
            tmpStr = []
            tmpStr.append([uid, str(normalize(i)), str(normalize(userInfoList[0])), str(normalize(userInfoList[1])),
                           str(normalize(userInfoList[2])), str(normalize(userInfoList[3])),
                           str(normalize(userInfoList[4])), str(normalize(userInfoList[5])),
                           normalize(str(userInfoList[6]).encode('utf8'))])
            try:
